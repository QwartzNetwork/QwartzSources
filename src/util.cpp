...
const char * const BITCOIN_CONF_FILENAME = "qwartz.conf";
const char * const BITCOIN_PID_FILENAME = "qwartzd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "qwartz" is a composite category enabling all Qwartz-related debug output
                if(ptrCategory->count(std::string("qwartz"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Qwartz
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Qwartz
    // Mac: ~/Library/Application Support/Qwartz
    // Unix: ~/.qwartz
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qwartz";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Qwartz";
#else
    // Unix
    return pathRet / ".qwartz";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty qwartz.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override qwartz.conf

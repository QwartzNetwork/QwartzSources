...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in qwartz.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("qwartz.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Qwartz features */
    { "qwartz",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "qwartz",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "qwartz",               "gobject",                &gobject,                true,  {} },
    { "qwartz",               "voteraw",                &voteraw,                true,  {} },

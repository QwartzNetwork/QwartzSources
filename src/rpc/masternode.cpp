...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "outputs") {
        ...
        pwalletMain->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_1200);
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Qwartz address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "qwartz",               "masternode",             &masternode,             true,  {} },
    { "qwartz",               "masternodelist",         &masternodelist,         true,  {} },
    { "qwartz",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "qwartz",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "qwartz",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "qwartz",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};

...
Qwartz Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7531 for connections and services them.
...
However, in open source software such as Qwartz Core, where everyone uses
...
pay attention to for reviewers of Qwartz Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Qwartz Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Qwartz Core so that they can be integrated
